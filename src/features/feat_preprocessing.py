...
def _inverse_transform(x):
    return 1 / np.where(x == 0, 1e-8, np.abs(x))

def _square_transform(x):
    return x ** 2
...
class FeatureTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, config, target_col, alpha=0.2):
        ...
        self.transform_functions = {
            'Linear': _linear_transform,
            'Log': _log_transform,
            'Sqrt': _sqrt_transform,
            '1/x': _inverse_transform,
            'square_func': _square_transform
        }             
